...
def read_group(assignment, group_heterability):
    f1 = open(assignment, 'r')
    f2 = open(group_heterability, 'r')
    sigma_assignment = []
    for line in f1:
        line = line.strip()
        temp = [int(line),0]
...
if __name__ == "__main__":
    ...
    parser.add_argument('-r', '--rho-prob', required=False, dest='rho_probability',
                        help='set rho probability, default is 0.95')
    ...
    parser.add_argument('-a', '--study_assignment', required=False, dest='Assignment',
                        help='list of assignments to genetic group for each study')
    parser.add_argument('-g', '--group_specific_heterability', required=False, dest='Sigma_squared',
                        help='list of heterogeneity for specific genetic group (e.g. African, Asian, European, etc.), default is 0')
    ...
    if args.rho_probability:
        rho_prob = float(args.rho_probability)
    else:
        rho_prob = 0.95
    ...
    if args.Sigma_squared and args.Assignment:
