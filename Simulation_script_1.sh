...
for i in {1..10};do
	python simulate.py -l sample_data/50_LD.txt -z sample_data/50_Z.txt
	python MCaviar.py -l LD -z Z -o $simulation'/'$outfile'_'$i -r $this_rho
	rm -r LD
	rm -r Z

	speculate_file=$outfile'_'$i'_set.txt'
	python capture.py -s $simulation'/'$speculate_file -t true_causal_set.txt -p $simulation
done
